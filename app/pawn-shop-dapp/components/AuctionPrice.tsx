...
export const AuctionPrice = ({ auction, global }: { auction: AuctionData, global: GlobalData | null }) => {
    ...
    useEffect(() => {
        ...
        const timerId = setInterval(() => {
            ...
            let time = Math.floor(Math.max(new Date().getTime() - auction.startedTime.toNumber() * 1000) / (global.decreaseInterval * 1000));
    ...
    return (
        <>
            <div className="flex items-center justify-center text-center text-[16px] font-semibold">
                Started At: {moment(new Date(auction.startedTime.toNumber() * 1000)).fromNow()}
            </div>
            <div className="flex items-center justify-center text-center text-[16px] font-semibold">
            ...
            {auction.finishedTime.toNumber() === 0 ?
                ...
                <>
                    ...
                    <div className="flex items-center justify-center text-center text-[16px] font-semibold">
                        Finished At: {moment(new Date(auction.finishedTime.toNumber() * 1000)).fromNow()}
                    </div>
                    <div className="flex items-center justify-center text-center text-[16px] font-semibold">
